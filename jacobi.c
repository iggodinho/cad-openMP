...
int run(double *A, double *b, double *x, double *xtmp)
{
  ...
  do
  {
    ...
    #pragma omp parallel for private(col, dot) shared(A, x, b, xtmp) reduction(+:sqdiff)
    for (row = 0; row < N; row++)
    {
      dot = 0.0;
      for (col = 0; col < N; col++)
...
int main(int argc, char *argv[])
{
  ...
  if (NUM_THREADS > 0) {
    omp_set_num_threads(NUM_THREADS);  // Defina o número de threads desejado
  }
...
void parse_arguments(int argc, char *argv[])
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    {
      if (++i >= argc || (NUM_THREADS = parse_int(argv[i])) < 0)
      {
